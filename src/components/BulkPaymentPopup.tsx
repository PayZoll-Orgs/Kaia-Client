...
interface PaymentContact extends Contact {
  amount: number;
}

interface BulkPaymentPopupProps {
  isOpen: boolean;
  onClose: () => void;
  onConfirmBulkPayment: (contacts: PaymentContact[], totalAmount: number) => void;
}
...
export default function BulkPaymentPopup({ isOpen, onClose, onConfirmBulkPayment }: BulkPaymentPopupProps) {
  const [step, setStep] = useState<'select' | 'preview'>('select');
  const [selectedContacts, setSelectedContacts] = useState<PaymentContact[]>([]);
  ...
  const handleContactToggle = (contact: Contact) => {
    ...
    if (isSelected) {
      ...
      setSelectedContacts(prev => [...prev, { ...contact, amount: 0 }]);
    }
  };

  const handleAmountChange = (contactId: string, amount: number) => {
  ...
  const totalAmount = selectedContacts.reduce((sum, contact) => sum + contact.amount, 0);
  const hasValidAmounts = selectedContacts.every(contact => contact.amount > 0);

  const handleConfirmBulkPayment = () => {
    if (hasValidAmounts) {
      onConfirmBulkPayment(selectedContacts, totalAmount);
      onClose();
    }
  };
              ...
              <Dialog.Panel className="w-full max-w-md transform overflow-hidden rounded-2xl bg-white shadow-xl transition-all">
                ...
                <div className="flex items-center justify-between p-6 border-b border-gray-200">
                  <Dialog.Title as="h2" className="text-gray-900 text-xl font-bold">
                    {step === 'select' && 'Select Recipients'}
                    {step === 'preview' && 'Preview Payment'}
                  </Dialog.Title>
                  <button
                    onClick={onClose}
        ...
        {step === 'select' && (
          <>
            ...
            <div className="p-6 border-t border-gray-200">
              ...
              >
                Preview Payment ({selectedContacts.length})
              </button>
            </div>
          </>
        )}
                  ...
                  <div key={contact.id} className="bg-gray-50 rounded-xl p-4 border border-gray-100">
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        onChange={(e) => handleAmountChange(contact.id, Number(e.target.value))}
