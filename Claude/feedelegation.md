...
User signs the transaction - User creates and signs a fee-delegated transaction with their wallet
DApp sends to Fee Delegation Service - Your DApp backend sends the signed transaction to Fee Delegation Service API endpoint
Server verifies and pays fees - Fee Delegation Service validates the transaction and pays gas fees on behalf of the user if it is valid
Fee Delegation Service Workflow

2. Apply for Access

You can request access to the Kaia Fee Delegation Service by reviewing and completing this Google form.
...
With API Key: You provide a valid API key AND either your contract or sender address is whitelisted (any sender & contract address will work with a valid API key if no whitelist is configured)
Without API Key: Either your contract or sender address is whitelisted in a DApp that doesn't have API keys
...
ethers-ext@V6 HTML example
ethers-ext@V6 ReactJS example
web3js example
viem-ext example
6. API Reference
...
Sending transaction was failed after 5 try, network is busy. Error message: <ERROR MESSAGE>	1. Verify that the userSignedTx.raw payload is correctly formatted and signed
2. For Low Nonce issue, ensure there are no pending transactions in your wallet. (For Kaia wallet, cancel any pending transactions and clear your history before retrying).
3. For KNOWN transaction issue, recheck if your current transaction is completed and move to other transactions
...
Wait for confirmation: Ensure each transaction is confirmed (i.e., you’ve received the transaction receipt) before sending the next one.
Ensure nonces are managed correctly: ensuring transactions are submitted with the correct nonce and that any failed or older nonces are properly handled before new transactions proceed
Implement retry logic: Build a retry mechanism to handle temporary failures, dropped transactions, or delayed confirmations.
Frontend wallet users: If using browser-based wallets (like Kaia Wallet, OKX Wallet, or Bitget) from the frontend, advise users to clear any pending or stuck transactions by using the wallet’s “Clear History” feature to prevent nonce conflicts.
API Calls
...
Kaia Fee Delegation Example
DApp Portal Fee Delegation
Kaia SDK Documentation
Custom Integrations
...
A: To continue using the service, you will need to contact the Kaia team to deposit or request more balance. However, you can also make sure to call /api/balance?address=${address} API to check if you have sufficient balance and check from the dashboard as well https://fee-delegation.kaia.io/rank
