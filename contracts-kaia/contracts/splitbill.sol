...
/**
 * @title SplitBilling  
 * @dev Contract for creating split bills where each person pays their own share directly to payee using USDT
...
contract SplitBilling is ReentrancyGuard {
    ...
    function createSplit(
        ...
        emit SplitCreated(splitId, msg.sender, payee, address(usdtToken), totalAmount, deadline, description, debtors, amounts);
    }
     ...
     */
    function payShare(uint256 splitId) external nonReentrant {
        ...
        // Transfer USDT directly to payee  
        usdtToken.transferFrom(msg.sender, split.payee, owedAmount);
        ...
        emit PaymentMade(splitId, msg.sender, address(usdtToken), owedAmount, split.totalPaid);

        // Check if split is now complete (no need to transfer, already done above)
        if (split.totalPaid == split.totalAmount) {
            emit SplitCompleted(splitId, split.payee, address(usdtToken), split.totalAmount);
        }
    }
     ...
     */
    function payForSomeone(uint256 splitId, address debtor) external nonReentrant {
        if (splitId == 0 || splitId > splitCounter) revert SplitNotFound();
        ...
        emit PaymentMade(splitId, debtor, address(usdtToken), owedAmount, split.totalPaid);
